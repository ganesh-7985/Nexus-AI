...
class GeminiLLM(BaseLLM):
    ...
    async def _achat_completion(
        ...
        resp: AsyncGenerateContentResponse = await self._retry_generate(messages, stream=False)
        usage = await self.aget_usage(messages, resp.text)
        self._update_costs(usage)
        return resp

    async def _retry_generate(self, messages: list[dict], stream: bool = False, max_retries: int = 5):
        """Retry generate_content_async with exponential backoff for transient errors."""
        for attempt in range(max_retries):
            try:
                return await self.llm.generate_content_async(**self._const_kwargs(messages, stream=stream))
            except Exception as e:
                err_str = str(e).lower()
                is_transient = any(k in err_str for k in ["503", "unavailable", "overloaded", "high demand", "rate", "resource_exhausted", "429"])
    ...
    async def _achat_completion_stream(self, messages: list[dict], timeout: int = USE_CONFIG_TIMEOUT) -> str:
        resp: AsyncGenerateContentResponse = await self._retry_generate(messages, stream=True)
        collected_content = []
