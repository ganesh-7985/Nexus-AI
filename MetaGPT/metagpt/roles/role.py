...
# HumanProvider removed — stub for compatibility
class _HumanProviderStub:
    pass
HumanProvider = _HumanProviderStub
...
# Planner removed — lightweight stub for compatibility
class _PlanStub(BaseModel):
    """Serializable stand-in for the real Plan object."""
...
class Planner(BaseModel):
    """Minimal stub replacing metagpt.strategy.planner.Planner."""
    model_config = ConfigDict(arbitrary_types_allowed=True)
    goal: str = ""
    working_memory: Optional[object] = Field(default=None, exclude=True)
    auto_run: bool = False
    plan: _PlanStub = Field(default_factory=_PlanStub)
    current_task: Optional[object] = Field(default=None, exclude=True)
    ...
    def get_useful_memories(self):
        return [Message(content="Plan complete")]
