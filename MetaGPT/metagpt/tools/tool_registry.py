...
@Desc    : Simplified tool registry (tool_convert + tool_data_type removed).
"""
...
# ── Lightweight stubs for removed tool_data_type ────────────────────────
...
def make_schema(tool_source_object, include, path):
    """Build a minimal schema from the source object via introspection."""
    schema = {}
    if tool_source_object is None:
        return schema
    try:
        if inspect.isclass(tool_source_object):
            methods = {}
            for name, method in inspect.getmembers(tool_source_object, predicate=inspect.isfunction):
                if include and name not in include:
                    continue
                if name.startswith("_"):
                    continue
                methods[name] = {"description": (inspect.getdoc(method) or "")[:200]}
            schema = {"class_name": tool_source_object.__name__, "methods": methods}
        elif callable(tool_source_object):
            schema = {"function_name": tool_source_object.__name__, "description": (inspect.getdoc(tool_source_object) or "")[:200]}
...
def register_tools_from_file(file_path) -> dict[str, Tool]:
    """Register tools from a Python file (simplified without AST converter)."""
    file_name = Path(file_path).name
    if not file_name.endswith(".py") or file_name == "setup.py" or file_name.startswith("test"):
        return {}
    # Without tool_convert, we can only register by decorator; skip file-level scanning
    return {}
