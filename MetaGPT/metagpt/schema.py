...
# repo_parser removed — stub DotClassInfo
class DotClassInfo:
    """Stub replacing metagpt.repo_parser.DotClassInfo."""
...
class Message(BaseModel):
    ...
    async def parse_resources(self, llm: "BaseLLM", key_descriptions: Dict[str, str] = None) -> Dict:
        ...
        try:
            ...
            pass
        # Attempt 2: find a JSON object containing "resources" key
        if m is None:
            import re
            for candidate in re.finditer(r'\{[^{}]*"resources"[^{}]*(?:\{[^{}]*\}[^{}]*)*\}', rsp, re.DOTALL):
                try:
                    m = json.loads(candidate.group())
                    break
                except (json.JSONDecodeError, ValueError):
                    continue
        # Attempt 3: give up gracefully
        if m is None or not isinstance(m, dict):
            logger.warning(f"parse_resources: Could not parse LLM response, using empty resources")
            m = {"resources": [], "reason": "LLM response was not valid JSON"}
        m.setdefault("resources", [])
        m["resources"] = [Resource(**i) for i in m.get("resources", []) if isinstance(i, dict)]
        return m
