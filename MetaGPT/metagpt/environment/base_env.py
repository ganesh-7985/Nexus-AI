...
try:
    ...
    # gymnasium not installed — lightweight stubs
    class _Space:
...
class BaseEnvAction:
    pass
class BaseEnvObsParams:
    pass
class EnvAPIAbstract:
    api_name: str = ""
    args: tuple = ()
    kwargs: dict = {}
class _APIRegistry(dict):
    def get_apis(self): return list(self.values())
ReadAPIRegistry = _APIRegistry
WriteAPIRegistry = _APIRegistry
...
class Environment(ExtEnv):
    ...
    def publish_message(self, message: Message, peekable: bool = True) -> bool:
        ...
        found = False
        msg_send_to = getattr(message, 'send_to', set())
        print(f"[ENV] publish_message: send_to={msg_send_to}, cause_by={getattr(message, 'cause_by', '')}")
        # According to the routing feature plan in Chapter 2.2.3.2 of RFC 113
        for role, addrs in self.member_addrs.items():
            matched = is_send_to(message, addrs)
            role_name = getattr(role, 'name', '') or getattr(role, 'profile', '')
            if matched:
                print(f"[ENV]   → routed to {role_name} (addrs={addrs})")
                role.put_message(message)
                found = True
        if not found:
            print(f"[ENV]   → NO RECIPIENTS! member_addrs keys: {[(getattr(r,'name','') or getattr(r,'profile','')) for r in self.member_addrs.keys()]}")
