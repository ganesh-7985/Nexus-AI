...
class Team(BaseModel):
    ...
    async def run(self, n_round=3, idea="", send_to="", auto_archive=True):
        ...
        while n_round > 0:
            # Debug: log role idle status each round
            idle_status = {r.name or r.profile: r.is_idle for r in self.env.roles.values()}
            print(f"[TEAM] Round check — idle_status: {idle_status}")
            if self.env.is_idle:
                print("[TEAM] All roles idle — exiting loop.")
                break
            n_round -= 1
            self._check_balance()
            await self.env.run()
            print(f"[TEAM] Round done — {n_round} rounds left.")
