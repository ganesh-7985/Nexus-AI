...
def register_action_outcls(func):
    """No-op decorator replacing action_outcls_registry."""
    return func


def exp_cache(**kwargs):
    """No-op decorator replacing exp_pool.exp_cache."""
    def wrapper(fn):
        return fn
    return wrapper


class _ActionNodeSerializerStub:
    pass
ActionNodeSerializer = _ActionNodeSerializerStub


def llm_output_postprocess(output: str, schema: dict, req_key: str = "") -> dict:
    """Extract JSON from LLM output that may be wrapped in [CONTENT]...[/CONTENT] or ```json...``` blocks."""
    import json as _json
    import re as _re

    # Strategy 1: Extract from [CONTENT]...[/CONTENT] tags
    content_match = _re.search(r'\[CONTENT\]\s*(.*?)\s*\[/CONTENT\]', output, _re.DOTALL)
    if content_match:
        try:
            return _json.loads(content_match.group(1))
        except _json.JSONDecodeError:
            pass

    # Strategy 2: Extract from ```json...``` code blocks
    json_block = _re.search(r'```json\s*(.*?)\s*```', output, _re.DOTALL)
    if json_block:
        try:
    ...
    # Strategy 3: Find the largest JSON object in the output
    # Look for { ... } that spans most of the text
    brace_match = _re.search(r'\{[\s\S]*\}', output)
    if brace_match:
        try:
    ...
    try:
        ...
        logger.error(f"llm_output_postprocess: Could not parse JSON from output (first 200 chars): {output[:200]}")
        return {}


class HumanInteraction:
    """Stub replacing utils.human_interaction."""
    def interact_with_instruct_content(self, instruct_content=None, mapping=None, interact_type="review"):
        return {}
